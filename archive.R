...
prepare_auth <- function(){
  message("In order to utilise web cookie authorisation, it is necessary to copy cookies\n from an active web session.\n")
  ...
  cookies <- paste0('\"logged-in-sig=', login_sig,';logged-in-user=',login_user_cookie,';\"')
  return(cookies)
}
...
pass_query <- function(query_url, cookies){
  ...
  query <- paste0('curl -X GET -H "Accept: application/json" --cookie ',cookies," ", save_url, query_url,'/&capture_outlinks=1/&js_behavior_timeout=5')
system(query)
}
...
for (steps in seq(from =1, to = length(url_list), by = 5)){
start <- steps
end <- ifelse( (steps + 4) > length(url_list), length(url_list), steps + 4)
